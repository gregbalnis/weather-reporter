# weather-reporter Development Guidelines

Auto-generated from all feature plans. Last updated: 2025-12-26

## Active Technologies
- Go (Latest Stable) + Standard Library (`flag`, `net/http`, `encoding/json`, `bufio`), `github.com/stretchr/testify` (Testing), Open-Meteo API (External) (001-weather-by-location)
- Go 1.25.5 + `github.com/stretchr/testify` (002-increase-test-coverage)
- N/A (stateless CLI application) (004-use-meteo-sdk)
- Go 1.25.5 + `github.com/gregbalnis/open-meteo-weather-sdk` (004-use-meteo-sdk)

- Go (Latest Stable) + Standard Library (`flag`, `net/http`, `encoding/json`, `bufio`), Open-Meteo API (External) (001-weather-by-location)

## Project Structure

```text
src/
tests/
```

## Commands

# Add commands for Go (Latest Stable)

## Code Style

Go (Latest Stable): Follow standard conventions

## Recent Changes
- 004-use-meteo-sdk: Added Go 1.25.5 + `github.com/gregbalnis/open-meteo-weather-sdk`
- 004-use-meteo-sdk: Added Go 1.25.5
- 003-automated-delivery-pipeline: Added [if applicable, e.g., PostgreSQL, CoreData, files or N/A]


<!-- MANUAL ADDITIONS START -->
<!-- MANUAL ADDITIONS END -->
